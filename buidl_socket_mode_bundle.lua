-- buidl_socket_mode_bundle.lua
-- Main application using WebSocket Socket Mode (bundled with all dependencies)

local config_module = require('config')

-- Include all bundled modules
...
dofile('slack_socket_mode_bundle.lua')
...
local function main()
    ...
    -- Handle Ctrl+C gracefully (if signal module is available)
    local signal_available, signal = pcall(require, 'signal')
    if signal_available then
